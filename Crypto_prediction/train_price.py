...
def train_price(currency):
   ...
   if not os.path.isfile("finalized_" + currency + ".csv"):
    pickle.dump(rf_model, open("finalized_" + currency + ".csv", 'wb'))
...
def real_time_test(currency):
    ...
    if not os.path.isfile('real_time_result_' + currency + '_rf.csv'):
        final_real_time_df.to_csv('real_time_result_' + currency + '_rf.csv')
    else:
        os.remove('real_time_result_' + currency + '_rf.csv')
        final_real_time_df.to_csv('real_time_result_' + currency + '_rf.csv')

    load_model = pickle.load(open("finalized_" + currency + ".csv", 'rb'))
    ...
    if not os.path.isfile('predicted_result_' + currency + '_rf.csv'):
        predicted_df = pd.DataFrame()
        predicted_df["Date"] = days
        predicted_df["Price"] = result
        predicted_df.to_csv('predicted_result_' + currency + '_rf.csv')
    else:
        os.remove('predicted_result_' + currency + '_rf.csv')
        predicted_df = pd.DataFrame()
        predicted_df["Date"] = days
        predicted_df["Price"] = result
        predicted_df.to_csv('predicted_result_' + currency + '_rf.csv')
...
train_price('BinanceCoin')
real_time_test('BinanceCoin')
